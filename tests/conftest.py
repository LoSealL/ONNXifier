...
def classification_models():
    ...
    # github.com raw URLs serve LFS pointers since the model zoo moved its
    # weights to git-lfs; only media.githubusercontent.com has the binaries
    header = "https://media.githubusercontent.com/media/onnx/models/main/validated/vision/classification/"
    models = [
        ("mobilenet/model/mobilenetv2-10.onnx", None),
        ("resnet/model/resnet18-v2-7.onnx", None),
        ("resnet/model/resnet50-v1-12-qdq.onnx", None),
    ]
    return [(header + i, hash_value) for (i, hash_value) in models]
