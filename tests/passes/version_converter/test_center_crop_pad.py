...
def _make_center_crop_pad_op19(shape, axes):
    node = make_node("CenterCropPad", ["X", "shape"], ["Y"], "centercrop", axes=axes)
    graph = make_graph(
        [node],
        "graph",
        [make_tensor_value_info("X", onnx.TensorProto.FLOAT, [1, 8, 24, 24])],
        [make_tensor_value_info("Y", onnx.TensorProto.FLOAT, [1, "C", "H", "W"])],
        [from_array(np.array(shape, np.int64), "shape")],
    )
    model = make_model(
...
def test_downgrade_center_crop_pad_19_to_17_crop():
    graph = _make_center_crop_pad_op19([16, 16], axes=[2, 3])
...
def test_downgrade_center_crop_pad_19_to_17_pad():
    graph = _make_center_crop_pad_op19([32, 32], axes=[2, 3])
    runner_ref = Evaluator(graph.model, "onnx")
    x = np.random.uniform(0, 255, size=[1, 8, 24, 24]).astype(np.float32)
    y_ref = runner_ref(["Y"], {"X": x})[0]
...
def test_downgrade_center_crop_pad_19_to_17_crop_and_pad():
    graph = _make_center_crop_pad_op19([16, 32], axes=[2, 3])
    runner_ref = Evaluator(graph.model, "onnx")
    x = np.random.uniform(0, 255, size=[1, 8, 24, 24]).astype(np.float32)
    y_ref = runner_ref(["Y"], {"X": x})[0]
